...
UILoadFont(byte_string Name, f32 Size, render_handle BackendHandle, memory_arena *Arena)
{
    ...
    vec2_i32          TextureSize    = OSGetClientSize(); // BUG: This is slightly weird. Could lead to bugs and shouldn't it match the OS texture anyways?
    ...
    if (CreatedGlyphCache)
    {
        ...
        if (CreatedGlyphTransfer)
        {
            ...
            {
                // TODO: Probably get the rest of the information we need relating to placing the text.
            }
            else
            {
